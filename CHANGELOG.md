...
- **Queue Inspection, Seeking & Now-Playing Enrichment:** Added `homectl sonos queue` for paginated playback queue inspection, `homectl sonos seek` for track jumping and time seeking (`[H:]MM:SS`), exposed `seek_track` and `seek_time` actions on `sonos_control`, and enriched `sonos_get_now_playing` and CLI `now-playing` with coordinator `queue_length` and `media_uri` via `GetMediaInfo` ([control-ul9], [control-mdo], [control-b79]).
